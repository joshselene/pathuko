...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const openStory = vscode.commands.registerCommand('aiChangeStory.openStory', () => lastAnalysis ? view.show(lastAnalysis) : vscode.commands.executeCommand('aiChangeStory.analyze'));
  const refresh = vscode.commands.registerCommand('aiChangeStory.refresh', () => vscode.commands.executeCommand('aiChangeStory.analyze'));
  const showMap = vscode.commands.registerCommand('aiChangeStory.showMap', () => lastAnalysis ? view.show(lastAnalysis) : vscode.commands.executeCommand('aiChangeStory.analyze'));
  const showRisk = vscode.commands.registerCommand('aiChangeStory.showRisk', () => lastAnalysis ? view.show(lastAnalysis) : vscode.commands.executeCommand('aiChangeStory.analyze'));
